-- ver2: Add fyp (+ fyp_top_up)
-- ver3: Fix fyp_top_up (using Contest's rules)
-- ver4: Created curated layer
-- ver5: Changed selection criteria from sbmt_dt to pol_iss_dt
-- ver6: Add assign_agt_ind to filter non-assigned agents
--		 Add additional agent's personal information
-- ver7: Add criteria to exclude non-confirmed or confirmed > 21 days
-- ver8: Remove 3 months incentive limit (policies can be issued anytime <=1 year after campaign date)
-- ver9: Remove assign_agt filter but extend to location match instead
-- ver10: Add total APE = basic APE + topup APE
--		: Change offr_st_dt to batch_start_date for filter
-- ver11: Add FYC
--		: Add ePOS indicator
--		: Add RH name
-- ver12: Fix duplicated FYC
--		: Fix tgt_agt_code
--		: Add new_loc_code
-- ver13: Change and fix freelook period to <= 20 days
--		: Fix the blank new_pol_cnfrm_dt
--		: Change new_pol_cnfrm_dt to cnfrm_dt from tabd_sbmt_pols
-- ver14: Fix the duplicate record by removing the new_pol_cust_id checking in TRACKINGM
-- ver15: Add condition for RH
--		: Now include assign_sm_ind for same SM locations
-- ver16: Add condition for Sub RH
--		: Now include assign_sub_rh_ind for same Sub-RH locations
-- ver17: Add new policy PO to calculate conversion rate
-- ver18: Include assign_rh_ind for same RH location
...
SET hivevar:end_date='2023-12-31';		-- Date range customers buying new policies
SET hivevar:rpt_mth=2301;
		...
		(SELECT	c.cmpgn_id,
		 ...
		 FROM	(SELECT a.cmpgn_id,
								 ...
								 FROM	(SELECT	c.*, 
										 ...
										 WHERE	cmpgn_id LIKE 'MIL-%-20%'
											...
											AND	new_pol_plan_cd NOT IN ('FDB01','PN001','BIC01','BIC02','BIC03','BIC04')
										 	AND new_cvg_stat IN ('1','3')
										 	AND	(new_pol_cnfrm_dt IS NULL OR DATEDIFF(new_pol_cnfrm_dt,new_cvg_iss_dt)<=20) --[ver13]
										) a
								) t
						 WHERE	t.rn=1
						) b
				...
				WHERE	a.cmpgn_id LIKE 'MIL-%-20%'
					AND	a.batch_start_date>=${cmpgn_st_dt}	--[ver10]
					AND	b.new_cvg_iss_dt between a.batch_start_date and a.batch_end_date
					--AND DATEDIFF(COALESCE(b.new_pol_cnfrm_dt,CURRENT_DATE),b.new_cvg_iss_dt)<=20 --[ver13]
				GROUP BY		
						a.cmpgn_id,
				 		a.tgt_agt_regional_head,
						a.tgt_sub_rh_name,
			...
			WHERE DATEDIFF(COALESCE(d.cnfrm_dt,CURRENT_DATE),c.pol_iss_dt)<=20 --[ver13] 
			GROUP BY
				c.cmpgn_id,
	...
	INNER JOIN
		(SELECT	tgt_agt_code, COUNT(DISTINCT tgt_cust_id) no_leads	--[ver17]
		 ...
		 WHERE	cmpgn_id LIKE 'MIL-%-20%'
			...
			AND btch_id NOT LIKE '9%'
			AND target_channel='Agency'
			AND batch_start_date BETWEEN ${start_date} AND ${end_date}
		 GROUP BY
		 		tgt_agt_code
		) lead ON tgt.tgt_agt_code=lead.tgt_agt_code
WHERE	tgt.pol_iss_dt BETWEEN ${start_date} AND ${end_date}
	AND	(tgt.new_cvg_stat NOT IN ('A','N','X','8') OR tgt.new_cvg_stat IS NULL)
	--AND	tgt.new_pol_num IN ('3817853367')
;
