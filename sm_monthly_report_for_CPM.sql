...
SET hivevar:ed_dt='2023-04-30';
SET hivevar:lead_yr=2023;

----- CPM Leads -----
SELECT	loc.rh_name rh_name,
        loc.sm_name sm_name,
		loc.loc_cd loc_code,
...
FROM	vn_published_campaign_db.targetm_dm tgt
	inner join
		(select	DISTINCT
				loc_cd, NVL(manager_name_0,NVL(manager_name_1,NVL(manager_name_2,NVL(manager_name_3,NVL(manager_name_4,NVL(manager_name_5,NVL(manager_name_6,rh_name))))))) sm_name,
                rh_name
		 from	vn_published_reports_db.loc_to_sm_mapping_hist
		 where 	image_date=${ed_dt}
		) loc
	 on	tgt.tgt_agt_loc_code=loc.loc_cd
...
SELECT	loc.rh_name rh_name,
        loc.sm_name sm_name,
		loc.loc_cd loc_code,
		LPAD(MONTH(tgt.BATCH_START_DATE),2,'00') month,
		COUNT(DISTINCT TGT_AGT_CODE) joined_agt,
		COUNT(DISTINCT TGT_CUST_ID) leads
FROM	vn_published_campaign_db.targetm_dm tgt
	inner join
	 ...
	 on	tgt.tgt_agt_loc_code=loc.loc_cd
WHERE	batch_start_date BETWEEN ${st_dt} AND ${ed_dt}
	AND	cmpgn_id LIKE 'MIL-%-2023%'
	AND SUBSTR(cmpgn_id,9,3) NOT IN ('POC','RAN')
	AND	btch_id not like '9%'
GROUP BY									   
	loc.rh_name,
    loc.sm_name,
	loc.loc_cd,
	LPAD(MONTH(tgt.BATCH_START_DATE),2,'00')

----- APE SM Locations ---
select	loc_cd, NVL(manager_name_0,NVL(manager_name_1,NVL(manager_name_2,NVL(manager_name_3,NVL(manager_name_4,NVL(manager_name_5,NVL(manager_name_6,rh_name))))))) sm_name,
        rh_name
from	vn_published_reports_db.loc_to_sm_mapping_hist
where 	image_date=${ed_dt}

----- CPM APE Performance -----
SELECT	LOC.RH_NAME region,
        LOC.SM_NAME sm,
		LOC.LOC_CD loc_code,
		...
		(SELECT	a.*,
		 ...
		 FROM	(SELECT	DISTINCT
		 		 ...
		 		 WHERE	trk.cmpgn_id NOT LIKE 'CTR-%'
				 	AND SUBSTR(trk.cmpgn_id,9,3) NOT IN ('POC','RAN')
					...
					AND	trk.new_pol_plan_cd NOT IN ('FDB01','BIC01','BIC02','BIC03','BIC04','PN005')
...
FROM	(SELECT	tgt_agt_regional_head,
		 ...
		 WHERE	cmpgn_id LIKE 'MIL-%-202%'
			...
			AND	btch_id NOT LIKE '9%'
		GROUP BY tgt_agt_regional_head, tgt_agt_loc_code, cmpgn_id, tgt_agt_code
		) tgt
		INNER JOIN
		(SELECT	DISTINCT
				loc_cd, NVL(manager_name_0,NVL(manager_name_1,NVL(manager_name_2,NVL(manager_name_3,NVL(manager_name_4,NVL(manager_name_5,NVL(manager_name_6,rh_name))))))) sm_name,
		 ...
		 FROM	vn_published_reports_db.loc_to_sm_mapping_hist
		 WHERE 	image_date=${ed_dt}
		) loc ON tgt.tgt_agt_loc_code=loc.loc_cd
		INNER JOIN
...
GROUP BY LOC.RH_NAME, LOC.SM_NAME, LOC.LOC_CD, p2.month
...
GROUP BY LOC.RH_NAME, LOC.SM_NAME, LOC.LOC_CD, LPAD(MONTH(dtk.txn_date),2,'00')
HAVING region_head IS NOT NULL
ORDER BY system